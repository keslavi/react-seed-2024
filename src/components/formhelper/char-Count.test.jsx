...
describe("Formhelper-CharCount", () => {
    ...
    render(
      <TestHarness item={{}}>
        <Input
          ...
          label="Description"
          charCount={50}
          data-testid="description-charcount"
        />
      </TestHarness>
    );
  ...
  it("shows character count in red when exceeding the limit", async () => {
    ...
    // The character count text should be red - target the span element specifically
    const charCountElement = screen.getByText((content, element) => {
      return element?.textContent === '60 / 20 characters' && element?.tagName === 'SPAN';
    });
    expect(charCountElement).toHaveStyle({ color: color.primary.red });
  ...
  it("prevents form submission when character count exceeds limit", async () => {
    ...
    // Check that the submit results input is empty (no submission occurred)
  ...
  it("allows form submission when character count is within limit", async () => {
    ...
    await user.type(input, 'This is a short message');
    
    // Submit the form
    await user.click(submitButton);
    
    // Check that the submit results input shows the submitted data
    await waitFor(() => {
      const submitResults = screen.getByLabelText('submit results');
  ...
  it("shows yup validation error when letter 'g' is required but missing", async () => {
    ...
    await user.type(input, 'This text does not contain the required letter');
  ...
  it("allows form submission when yup validation passes", async () => {
    ...
    await user.type(input, 'This text contains the required letter g');
  ...
  it("shows both character count error and yup validation error", async () => {
    ...
    await user.type(input, 'This is a very long message that exceeds the character limit and does not contain the required letter');
    
    // Check character count error - the text should be red - target the span element specifically
    const charCountElement = screen.getByText((content, element) => {
      return element?.textContent === '101 / 20 characters' && element?.tagName === 'SPAN';
    });
    expect(charCountElement).toHaveStyle({ color: color.primary.red });
  ...
  it("handles char count input with placeholder", () => {
    render(
      <TestHarness item={{}}>
        <Input
          name="description"
    ...
    const input = screen.getByRole('textbox', { name: /description/i });
    expect(input).toBeVisible();
    expect(input).toHaveAttribute('placeholder', 'Enter your description here');
  });

  it("handles char count input with info prop", () => {
    render(
      <TestHarness item={{}}>
        <Input
          name="description"
          label="Description"
          charCount={50}
          info="This is helpful information about the char count input"
          data-testid="description-charcount"
        />
      </TestHarness>
    );
    ...
    waitFor(() => {
      ...
      expect(popover).toHaveTextContent('This is helpful information about the char count input');
    });
  });

  it("handles char count input with input-specific props", () => {
    render(
      <TestHarness item={{}}>
        <Input
          name="description"
